...
_PATTERN = re.compile(
    r"(?:invoice|inv)\s+(?!balance|total|date|number|details|from|to|for|zoom|#)([A-Z]*\d+[A-Z0-9\-]*)",
    re.IGNORECASE,
)
_DIGIT_PATTERN = re.compile(r"\d")


class InvoiceSpaceExtractor(IInvoiceNumberExtractor):
    @property
    def Name(self) -> str:
        return "Invoice/INV + space"
