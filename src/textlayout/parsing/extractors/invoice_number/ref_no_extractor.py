...
_PATTERN = re.compile(r"(?:ref\.?\s*(?:no\.?|#)?)[:\s]*([A-Z]*\d+[A-Z0-9\-]*)", re.IGNORECASE)
_DIGIT_PATTERN = re.compile(r"\d")


class RefNoExtractor(IInvoiceNumberExtractor):
    @property
    def Name(self) -> str:
        return "Ref No/#"
    ...
    def ExtractAll(self, context):
        ...
        for match in matches:
            ...
            if len(num) >= 4 and _DIGIT_PATTERN.search(num):
                results.append(ExtractionResult(num, 1, match.group(0)))
